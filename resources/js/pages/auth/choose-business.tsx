...
import { Package, Check, Info, LoaderCircle, Sparkles, Zap } from 'lucide-react';
        ...
        <div className="mb-10 flex items-center justify-center gap-3">
            {[1, 2].map((s) => (
                <div key={s} className="flex items-center gap-3">
                    ...
                    <span
                        className={[
                            'text-sm font-bold hidden sm:block transition-colors',
                    ...
                    {s < 2 && (
                        <div className={['h-1 rounded-full w-8 transition-colors duration-300', step > 1 ? 'bg-white' : 'bg-[#1c1c1e]'].join(' ')} />
                    )}
                </div>
            ))}
        </div>
...
export default function ChooseBusiness({ plans = [] }: Props) {
    ...
    return (
        <>
            ...
            <div className="bg-black text-white min-h-svh w-full font-sans selection:bg-white/30 pb-20">
                ...
                <div className="relative mx-auto flex max-w-5xl flex-col items-center px-4 py-10 sm:py-16">
                    ...
                    <div className="mb-6 text-center">
                        ...
                        <p className="mt-3 text-sm text-gray-400 sm:text-base font-medium">
                            {step === 1
                    ...
                    {step === 1 && (
                        <div className="w-full space-y-12">
                            ...
                            <div className="grid grid-cols-1 gap-8 sm:grid-cols-3 lg:gap-12 mt-8">
                                {plans.map((plan) => {
                                    ...
                                    const scaleClass = isPro 
                                        ? (isSelected ? 'scale-[1.05] sm:scale-110 shadow-2xl' : isHovered ? 'scale-[1.02] sm:scale-[1.08] shadow-xl' : 'scale-100 sm:scale-105 shadow-lg')
                                        : (isSelected ? 'scale-[1.03] shadow-2xl' : isHovered ? 'scale-[1.01] shadow-xl' : 'scale-100 shadow-sm');
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={[
                                                'relative flex flex-col rounded-[2.5rem] p-7 text-left transition-all duration-300 cursor-pointer outline-none focus-visible:ring-offset-4 focus-visible:ring-offset-black',
                                                meta.bg,
                                                meta.text,
                                                scaleClass,
                                                isPro ? 'z-10' : 'z-0',
                                        ...
                                        >
                                            ...
                                            <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-[2.5rem]">
                                                <div className="absolute -right-6 -top-10 h-36 w-36 rounded-full bg-white/20 blur-2xl" />
                                            </div>

                                            {/* Badge - diletakkan di luar overflow-hidden agar terlihat jelas */}
                                            {meta.badge && (
                                                <span className={['absolute -top-5 left-1/2 -translate-x-1/2 rounded-full px-5 py-1.5 text-[11px] font-extrabold uppercase tracking-widest whitespace-nowrap shadow-xl z-20', meta.badgeColor].join(' ')}>
                                                    {meta.badge}
                                                </span>
                                            )}
                                            ...
                                            {/* Selected checkmark */}
                                            {isSelected && (
                                                <span className="absolute top-6 right-6 flex h-8 w-8 items-center justify-center rounded-full bg-black/20 text-current backdrop-blur-sm z-20 shadow-inner">
                                                    <Check className="h-5 w-5" />
                                                </span>
                                            )}

                                            {/* Plan icon */}
                                            <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-black/10 backdrop-blur-md z-10">
                                                {meta.icon}
                                            </div>
                                            ...
                                            {/* Limits */}
                                            <div className={['mt-4 flex gap-4 text-xs font-bold z-10', meta.descriptionText].join(' ')}>
                                                <span className="bg-black/5 px-2 py-1 rounded-md">👤 {plan.max_users === 99 ? 'Unlimited' : plan.max_users}</span>
                                                <span className="bg-black/5 px-2 py-1 rounded-md">📦 {plan.max_products >= 9999 ? 'Unlimited' : plan.max_products}</span>
                                            </div>

                                            {/* Features */}
                                            <ul className="mt-8 space-y-3 flex-1 z-10">
                                                {highlights.map((feat) => (
                                                ...
                                                {plan.features.length > highlights.length && (
                                                    <li className={['text-xs font-bold pl-8 pt-1', meta.descriptionText].join(' ')}>
                                            ...
                                            <div className="mt-8 pt-5 border-t border-black/10 z-10">
                                                <button
                                                    type="button"
                            ...
                            {errors.plan_slug && (
                                <p className="text-red-500 text-sm text-center font-bold">{errors.plan_slug}</p>
                            )}
                            ...
                            <div className="pt-4 flex justify-center">
                                <Button
                                    type="button"
                                    size="lg"
                                    className="w-full sm:w-2/3 h-16 rounded-full text-xl font-bold shadow-[0_0_40px_rgba(255,255,255,0.1)]"
                    ...
                    {step === 2 && (
                        <form onSubmit={handleSubmit} className="w-full space-y-8 max-w-4xl mx-auto">
                            {/* Back button */}
                            <button
                                type="button"
                                onClick={() => setStep(1)}
                            ...
                            {(() => {
                                ...
                                return p ? (
                                    <div className={['flex items-center gap-5 rounded-3xl p-5 transition-all shadow-lg', meta.bg, meta.text].join(' ')}>
                                        ...
                                        <div>
                                            ...
                                            <p className={['text-sm font-bold', meta.descriptionText].join(' ')}>
                                                {formatPrice(p.price)}{p.price > 0 ? '/bulan' : ''}
                                            </p>
                                        </div>
                            ...
                            <div className="space-y-3 pt-2">
                                ...
                                <Input
                                    ...
                                    className="h-16 rounded-full border-0 bg-[#1c1c1e] px-8 text-lg font-bold text-white placeholder:text-gray-600 placeholder:font-medium focus-visible:ring-4 focus-visible:ring-[#1a56ff]"
                                />
                                    ...
                                    {businessTypes.map((type) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="mt-1">
                                                    <p className="text-lg font-bold leading-tight">{type.label}</p>
                                                    <p className={['mt-1.5 text-sm font-medium leading-snug', isSelected ? type.activeDesc : 'text-gray-400'].join(' ')}>
                                                        {type.description}
                                                    </p>
                                                </div>
                                            </button>
                            ...
                            <div className="pt-6">
                                <Button
                                    type="submit"
                                    size="lg"
                                    className="w-full h-16 rounded-full text-xl font-bold bg-[#1a56ff] text-white hover:bg-[#1545cc] shadow-[0_8px_30px_0_rgba(26,86,255,0.4)]"
            ...
            <Dialog open={!!detailPlan} onOpenChange={(open) => !open && setDetailPlan(null)}>
                {detailPlan && (
                    <DialogContent className="border-[#2c2c2e] bg-[#1c1c1e] text-white sm:rounded-[2.5rem] max-w-md p-7">
                        <DialogHeader>
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <DialogTitle className="text-3xl font-extrabold capitalize text-white">
                                        {detailPlan.name}
                                    </DialogTitle>
                                    <DialogDescription className="text-base font-bold text-gray-400 mt-1">
                                        {formatPrice(detailPlan.price)}{detailPlan.price > 0 ? ' / bulan' : ' (Gratis)'}
                                    </DialogDescription>
                                </div>
                            </div>
                        </DialogHeader>
                        ...
                        <div className="space-y-7 py-5">
                            ...
                            <div className="grid grid-cols-2 gap-4 rounded-[1.5rem] border border-[#2c2c2e] bg-black/40 p-5 text-center shadow-inner">
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-widest text-gray-500">Maks. User</p>
                                    <p className="mt-1.5 font-extrabold text-white text-2xl">
                                        {detailPlan.max_users === 99 ? 'Unl' : detailPlan.max_users}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-widest text-gray-500">Maks. Produk</p>
                                    <p className="mt-1.5 font-extrabold text-white text-2xl">
                        ...
                        <DialogFooter className="sm:justify-center mt-2">
                            <Button
                                type="button"
                                size="lg"
                                className="w-full h-14 rounded-full font-bold bg-white text-black hover:bg-gray-200 text-lg shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                                onClick={() => {
